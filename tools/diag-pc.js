...
function formatReport(diag) {
  ...
  for (const c of diag.cards) {
    ...
    if (c.extracted) {
      const curSym = c.extracted.currency === 'EUR' ? '€' : (c.extracted.currency === 'GBP' ? '£' : '$');
      out += `\n  ─── Prezzi principali (${c.extracted.currency || 'USD'}) ───\n`;
      const order = ['ungraded','grade7','grade8','grade9','grade9_5','psa10','bgs10','cgc10'];
      for (const k of order) {
        if (c.extracted[k] != null) out += `  ${pad(k, 12)} ${curSym}${c.extracted[k]}\n`;
      }
      ...
      if (c.extracted.grades_from_listings && Object.keys(c.extracted.grades_from_listings).length) {
        ...
        for (const [key, data] of entries) {
          ...
          out += `  ${confBadge} ${pad(key, 10)} ${sym}${String(data.median).padStart(9)}  (${data.count} vend., range ${sym}${data.min}–${sym}${data.max})\n`;
        }
      }
    }
